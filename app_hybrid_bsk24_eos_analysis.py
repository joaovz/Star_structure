...
a2_max = 550**2                                                 # Maximum a2 parameter value [MeV^2]
...
B_max = 350**4                                                  # Maximum B parameter value [MeV^4]
...
HADRON_EOS_PROPERTIES = {                                       # Hadron EOS properties in GU, used to avoid calculations when the Hybrid EOS turns out to be a Hadron EOS
    "maximum_stable_rho_center": 1.6943274822619097e-09,
    "maximum_mass": 3365.6077505778694,
    "maximum_cs": 0.9628433594424015,
    "canonical_rho_center": 5.421187170531078e-10,
    "canonical_radius": 12588.892272333944,
    "canonical_lambda": 520.7100095638465,
    "maximum_k2_star_rho_center": 3.894827391067386e-10,
    "maximum_k2": 0.11753310447543014,
}
...
def generate_hybrid_stars(number_of_samples=10**4):
...
def analyze_hybrid_star_family(dataframe_row):
    ...
    bsk24_eos_rho_space = MAX_RHO * EOS_LOGSPACE
    ...
    hybrid_eos = HybridEOS(quark_eos, bsk24_eos, "data/BSk24_EOS.csv", bsk24_maximum_stable_rho_center, "HybridBSk24EOS")
...
def plot_parameter_space(mesh_size=1000, figure_path="figures/app_hybrid_eos"):
    ...
    ax.plot_surface(a2_1_2_3f_masked, a4_3f_masked, B_1_4_3f_lim_surface_masked, cmap=cm.Reds, rstride=10, cstride=10, alpha=0.8, label="$B_{lim}^{1/4}$", rasterized=True)
    ax.plot_surface(a2_1_2_2f_masked, a4_2f_lim_surface_masked, B_1_4_2f_masked, cmap=cm.Blues, rstride=10, cstride=10, alpha=0.8, label="$\\tilde{B}_{lim}^{1/4}$", rasterized=True)
...
def main():
    ...
    # Constants
    figures_path = "figures/app_hybrid_bsk24_eos/analysis"                          # Path of the figures folder
    dataframe_csv_path = "results"                                                  # Path of the results folder
    dataframe_csv_name = "hybrid_bsk24_eos_analysis.csv"                            # Name of the csv file with the results
    filtered_dataframe_csv_name = "hybrid_bsk24_eos_analysis_filtered.csv"          # Name of the csv file with the results after filtering with observation data restrictions
    ...
    (a2_masked, a4_masked, B_masked, parameter_dataframe) = generate_hybrid_stars(number_of_samples)
